...
app.MapPost("/sistema/usuario/registrar", async ([FromBody] Usuario usuario, [FromServices] AppDbContex ctx) =>
{
    ...
    try
    {
        ...
        return Results.Created($"/sistema/usuario/buscar/{usuario.Email}", usuario);
    }
    catch (Exception e)
    {
...
{
    var eventos = await ctx.Eventos.Include(e => e.Proprietario).ToListAsync();
...
{
    var evento = await ctx.Eventos.Include(e => e.Proprietario).FirstOrDefaultAsync(e => e.Id == id);
...
app.MapPost("/sistema/evento/criar", async ([FromBody] Evento evento, [FromServices] AppDbContex ctx) =>
{
    ...
    try
    {
        ...
        await ctx.SaveChangesAsync();
        return Results.Created($"/sistema/evento/buscar/{evento.Id}", evento);
    }
    catch (Exception e)
    {
        return Results.Problem(detail: e.Message);
...
{
    var evento = await ctx.Eventos.FindAsync(inscricao.EventoId);
    if (evento == null)
    {
        return Results.NotFound(new { mensagem = "Evento não encontrado." });
    }

    var usuario = await ctx.Usuarios.FindAsync(inscricao.UsuarioId);
    if (usuario == null)
    {
        return Results.NotFound(new { mensagem = "Usuário não encontrado." });
    }
    ...
    var totalInscricoes = await ctx.Inscricoes.CountAsync(i => i.EventoId == evento.Id);
    if (totalInscricoes >= evento.VagasMaximo)
    {
        return Results.BadRequest(new { mensagem = "Evento lotado." });
    }

    var usuarioInscrito = await ctx.Inscricoes.AnyAsync(i => i.EventoId == evento.Id && i.UsuarioId == usuario.Id);
    if (usuarioInscrito)
    {
    ...
    try
    {
        ...
        await ctx.SaveChangesAsync();
        return Results.Created($"/sistema/usuario/buscar-inscricao/{usuario.Id}/{evento.Id}", inscricao);
    }
    catch (Exception e)
    {
        return Results.Problem(detail: e.Message);
...
app.MapGet("/sistema/evento/listar-inscritos/{id}", async ([FromRoute] int id, [FromServices] AppDbContex ctx) =>
{
    ...
    var inscricoes = await ctx.Inscricoes
        .Include(i => i.Usuario)
...
{
    var inscricao = await ctx.Inscricoes
        .Include(i => i.Usuario)
        .Include(i => i.Evento)
